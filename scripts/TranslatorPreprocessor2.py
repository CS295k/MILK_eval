...
instructionToNumCmdsCounts = defaultdict(int)

# counts how many times 1 english instruction corresponds to N # of cmds
def getInstructionCounts(maxNGrams):
    global instructionToNumCmdsCounts
    tmp = defaultdict(int)
    totalGroups = 0
    for recipe_file in glob('../annotated_recipes/*.xml'):
        ...
        for curCmd, curText in zip(commands, originals):
            if (curCmd == 'create_ing' and finishedCreate == False):
                continue
            if (curCmd != 'create_ing' and finishedCreate == False):
                finishedCreate = True
            #elif (curCmd == 'create_ing' and finishedCreate == True):
            #    print "** new create_ing!"
            if (pastText != "" and curText != pastText):
                tmp[len(cmdsGroup)] += 1
                totalGroups += 1
                cmdsGroup = []

            cmdsGroup.append(curCmd)
            pastText = curText
        tmp[len(cmdsGroup)] += 1
        totalGroups += 1
    ...
    for k,v in tmp.iteritems():
        if (k <= maxNGrams):
            instructionToNumCmdsCounts[k] = float(v)/total
    print "complete instruction-to-ngram-counts:", tmp
    print "truncated and normalized instruction-to-ngram probs:",instructionToNumCmdsCounts

def getNGramCounts():
    global dataInputDir, testingPercentage, unigramCounts, bigramCounts, trigramCounts, quadCounts, unigramSingletonCounts, unigramSameCounts, bigramTogetherCounts, trigramTogetherCounts, quadTogetherCounts
