...
def strip_to_predicate_from_commands(recipe):
  return [(ot, [x[0] for x in anns]) for ot, anns in recipe]
...
def remove_create_ing_from_commands(recipe):
  return [(ot, anns) for (ot, anns) in recipe if set(a[0] for a in anns) != set(['create_ing'])]

def remove_create_tool(recipe):
  return [(ot, [a for a in anns if a != 'create_tool']) for (ot, anns) in recipe]

def remove_create_tool_from_commands(recipe):
  return [(ot, [a for a in anns if a[0] != 'create_tool']) for (ot, anns) in recipe]
